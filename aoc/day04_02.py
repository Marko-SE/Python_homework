...
def check_spot(text, i, j):
    ...
    down_diagonal = text[i-1][j+1] + text[i][j] + text[i+1][j-1]
    upper = (upper_diagonal == "MAS" or upper_diagonal == "SAM")
    down = (down_diagonal == "MAS" or down_diagonal == "SAM")
    if upper and down:
        return True
    else:
        return False
...
# action!
data = read_input('../aoc_data/day03_data.txt')
...
for i in range(1, n-1):
    for j in range(1, n-1):
        if data[i][j] == 'A':
            total_xmas += check_spot(data, i, j)
